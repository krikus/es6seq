...
describe('Seq drop', function() {
	it('should work with callback', function() {
		...
		expect(seq.drop(callback).toArray()).to.be.deep.equal([-1,-5,3,4,6]);
	});

	it('should be able to take numberic parameter', function() {
		let seq = new Seq(['cat', 'dog', 'mouse']);

		expect(seq.drop(2).toArray()).to.be.deep.equal(['mouse']);
	});
	...
	it('should work with empty Seq', function() {
		...
		expect(seq.drop(5).toArray()).to.be.deep.equal([]);
	});
	...
	it('should be able to take more elements than there are in seq', function() {
		...
		expect(seq.drop(how_many).toArray()).to.be.deep.equal([]);
		expect(seq.drop(positive_callback).toArray()).to.be.deep.equal([]);
	});
});
